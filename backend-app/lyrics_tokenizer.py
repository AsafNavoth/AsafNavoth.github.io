...
def tokenize_lyrics(text: str) -> list[tuple[str, dict]]:
    ...
    if not text or not text.strip():
        logger.debug("tokenize_lyrics: empty input")
        return []
    text = remove_english_letters(text)
    if not text:
        logger.debug("tokenize_lyrics: no text after removing English")
        return []
    logger.debug("tokenize_lyrics: input len=%d text_preview=%r...", len(text), text[:80])
    ...
    if jam is None and unique_words:
        logger.error("tokenize_lyrics: jamdict unavailable but have %d tokens", len(unique_words))
    ...
    if found_count == 0 and len(tokenized) > 0:
        global _jamdict
        logger.warning("tokenize_lyrics: 0 definitions for %d tokens - retrying with fresh jamdict", len(tokenized))
        _jamdict = None
        jam = _get_jamdict()
        if jam is not None:
            tokenized = [(word, _lookup_word(jam, word)) for word in unique_words]
            found_count = sum(1 for _, r in tokenized if r.get('found'))
    logger.info("tokenize_lyrics: %d/%d words have definitions", found_count, len(tokenized))
    return tokenized
